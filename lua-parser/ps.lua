  ...
  for i, a in ipairs(args) do
    ...
    elseif 'string' == type(a) then
      ...
      if self.psCommands[a] ~= nil then
        ...
        if self.stackDepth < cmd.pop then
        ...
        self.stackDepth = self.stackDepth - cmd.pop + cmd.push
...
function PS:emitLocalRvalue(index, id, pos)
  ...
  trace(string.format('evaluating local "%s" (index from bottom = %d, stack depth = %d) pos=%d', id, index, self.stackDepth, pos))
  -- Check for PostScript stack underflow
  assert(index < self.stackDepth)
  -- Emit PostScript
  self:write(string.format('%d index %% evaluate local "%s" as an rvalue pos=%d\n', self.stackDepth - index, id, pos))
end
...
function PS:emitLocalAssignment(index, id, pos)
  ...
  self:write(string.format('%d %d roll pop %d 1 roll %% assign to local "%s" pos=%d\n',
    self.stackDepth - index
  , self.stackDepth - index - 1
  , self.stackDepth - index - 1
  , id
  , pos))
end
...
function PS:emitFunctionReturn(numReturns, pos)
  ...
  self:write(string.format('counttomark 1 add 1 roll cleartomark'))
end
